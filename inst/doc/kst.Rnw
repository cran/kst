...
Finally, the \codefunind{kfringe} function allows for determining the inner and outer fringe of a knowledge structure or a knowledge state respectively. Fringes determine the symmetric difference between a given knowledge state and its neighbouring states. The symmetric difference between a given knowledge state and its predecessor states is referred to as \emph{inner fringe}. These items can be interpreted as most recently learned and may for example be subject to learner review or assessment. The symmetric difference between a given knowledge state and its immediate successor states is referred to as \emph{outer fringe}. These items may be considered those a learner should tackle next and therefore allow for creating personalized learning paths.
<<kfringe>>=
